...
# CORS详解
跨域是浏览器的限制，而非服务器端的限制，只是需要服务器端对来源进行确认，这是一种安全措施。
...
CORS支持所有类型的HTTP请求，而JSONP只支持GET请求。CORS的优势在于支持现代浏览器的全部功能，而JSONP适用于老式浏览器和不支持CORS的网站请求数据。
